...
public class MoldDestructionRecovererBehavior : BlockBehavior
{
	...
	private readonly AssetLocation MetalBits_partial = new(GlobalConstants.DefaultDomain, @"metalbit");
	...
	public override void OnBlockBroken(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, ref EnumHandling handling)
	{
		...
		var CoreAPI = world.Api;
		var ServerAPI = world.Api as ICoreServerAPI;
...
#if DEBUG
			world.Api.Logger.VerboseDebug("{0} Ingot Mold(s) with L {1} Units, R {2} Units", ingotMold.QuantityMolds,
				ingotMold.FillLevelLeft, ingotMold.FillLevelRight);
#endif
	...
	internal void SpawnMetalBits(IWorldAccessor world, BlockPos pos, int unitQuantity, string baseMetalCode)
	{
		...
		{
			var shavingQty = unitQuantity / shavingValue;
			var metalShavingsItem = world.Api.World.GetItem(MetalBits_partial.AppendPathVariant(baseMetalCode));
