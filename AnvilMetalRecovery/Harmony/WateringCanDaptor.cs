...
public class WateringCanDaptor
{
	...
	public static void OnHeldInteractStep(ref bool __result, float secondsUsed, ItemSlot slot, EntityAgent byEntity,
		BlockSelection blockSel, EntitySelection entitySel, BlockWateringCan __instance)
	{
...
public class WateringCanAssist
{
	...
	public readonly AssetLocation CoolSoundEffect = new(@"game", @"sounds/sizzle");

	private readonly SimpleParticleProperties steamParticles = new()
	{
		MinPos = new Vec3d(),
		AddPos = new Vec3d(),
		MinQuantity = 6,
	...
	private ICoreAPI CoreAPI { get; }
	private ICoreServerAPI ServerAPI => CoreAPI as ICoreServerAPI;
	private ICoreClientAPI ClientAPI => CoreAPI as ICoreClientAPI;
	private BlockWateringCan Original { get; }

	public void PerformBlockCooling(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel,
		EntitySelection entitySel)
	{
		...
		if (!slot.Empty && Original.GetRemainingWateringSeconds(slot.Itemstack) >= 0.1f)
		{
			...
			{
				var someBlockEntity = server
					? ServerAPI.World.BlockAccessor.GetBlockEntity(targetPos)
					: ClientAPI.World.BlockAccessor.GetBlockEntity(targetPos);
				...
				if (someBlockEntity is BlockEntityIngotMold)
				{
					...
					{
						if (server) CoolContents(ingotMold.ContentsRight);
						else GenerateSpecialEffects(blockSel.Position, blockSel.HitPosition, byEntity as EntityPlayer);
						ingotMold.MarkDirty();
					}
					else if (ingotMold.FillLevelLeft > 0 && ingotMold.TemperatureLeft > flashPointTemp)
					{
						if (server) CoolContents(ingotMold.ContentsLeft);
						else GenerateSpecialEffects(blockSel.Position, blockSel.HitPosition, byEntity as EntityPlayer);
						ingotMold.MarkDirty();
					}
				...
				if (someBlockEntity is BlockEntityToolMold)
				{
					...
					{
						if (server) CoolContents(toolMold.MetalContent);
						else GenerateSpecialEffects(blockSel.Position, blockSel.HitPosition, byEntity as EntityPlayer);
						toolMold.MarkDirty();
					}
				}
	...
	internal void GenerateSpecialEffects(BlockPos blockLoc, Vec3d aimPoint, EntityPlayer playerEntity)
	{
		...
		ClientAPI.World.PlaySoundAt(CoolSoundEffect, playerEntity, playerEntity.Player, false, volume: 0.5f);
	}
	...
	internal void CoolContents(ItemStack itemStack)
	{
		...
		if (temperature > 20f) //TODO: USE local AMBIENT Temp
			itemStack.Collectible.SetTemperature(CoreAPI.World, itemStack, temperature - temperature * coolRateDefault,
				false);
