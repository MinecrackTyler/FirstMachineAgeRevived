...
public class AnvilDaptor
{
	...
	private static void Prefix_OnSplit(Vec3i voxelPos, BlockEntityAnvil __instance)
	{
		...
		{
#if DEBUG
			anvil.Logger.VerboseDebug("{0}, Split into {1} @{2}, Total:{3}", anvil.OutputCode, anvil.BaseMetal,
				voxelPos, anvil.SplitCount);
#endif
			anvil.SplitCount++;
		}
	}
	...
	{
		var anvil = new SmithAssist(__instance);
		if (anvil.WorkMatchesRecipe()) anvil.IssueShavings(byPlayer);
	}
	...
	private static void Postfix_GetBlockInfo(IPlayer forPlayer, StringBuilder dsc, BlockEntityAnvil __instance)
	{
		...
		if (anvil.BaseMaterial != null && anvil.SplitCount > 0)
			dsc.AppendFormat("[ {0} ] : {1} × {2}\n", anvil.SplitCount,
				Lang.GetUnformatted($"game:item-metalbit-{anvil.BaseMetal}"), anvil.ShavingQuantity);
		else if (!anvil.IsShavable) dsc.AppendLine(Lang.GetUnformatted("fma:cant_recover"));
	}
...
internal class SmithAssist
{
	...
	private readonly AccessTools.FieldRef<BlockEntityAnvil, ItemStack> workItemStack_R =
		AccessTools.FieldRefAccess<BlockEntityAnvil, ItemStack>(@"workItemStack");

	private readonly AccessTools.FieldRef<BlockEntityAnvil, ItemStack> returnOnCancelStack_R =
		AccessTools.FieldRefAccess<BlockEntityAnvil, ItemStack>(@"returnOnCancelStack");
	...
	internal ILogger Logger => bea.Api.World.Logger;

	internal IWorldAccessor World => bea.Api.World;

	public static AssetLocation MetalShavingsCode => new(GlobalConstants.DefaultDomain, @"metalbit");
	...
	private AMRConfig CachedConfiguration => (AMRConfig)bea.Api.ObjectCache[MetalRecoverySystem._configFilename];

	public int SplitCount
	{
		get => bea.WorkItemStack?.Attributes.TryGetInt(splitCountKey) ?? 0;
		set => bea.WorkItemStack?.Attributes.SetInt(splitCountKey, value);
	}
		...
		get
		{
			if (bea.WorkItemStack?.Collectible?.FirstCodePart().Equals(@"ironbloom") == true
			    || (bea.SelectedRecipe != null
			        && PrefixMatcher(CachedConfiguration.BlackList, OutputCode)))
			{
#if DEBUG
				Logger.VerboseDebug("That ain't shavable: {0}", OutputCode);
#endif
				return false;
			}
	...
	public int ShavingQuantity =>
		(int)(Math.Round(SplitCount * CachedConfiguration.VoxelEquivalentValue) / shavingValue);

	internal IAnvilWorkable AnvilWorkpiece
	{
		get
		{
			if (bea.WorkItemStack != null && bea.WorkItemStack.Collectible is IAnvilWorkable)
				return bea.WorkItemStack.Collectible as IAnvilWorkable;
	...
	internal AssetLocation OutputCode => bea.SelectedRecipe.Output.Code;

	public string BaseMetal => this?.BaseMaterial?.Collectible.LastCodePart();
	...
	internal void IssueShavings(IPlayer byPlayer)
	{
		if (SplitCount > 0)
		{
			var shavingQty = ShavingQuantity;

			if (shavingQty > 0)
			{
				var metalShavingsItem = World.GetItem(MetalShavingsCode.AppendPathVariant(BaseMetal));
				...
				if (metalShavingsItem != null)
				{
					...
					if (byPlayer != null)
					{
						if (!byPlayer.InventoryManager.TryGiveItemstack(metalShavingsStack))
...
#if DEBUG
					Logger.VerboseDebug("RecoveryAnvil: Smithing done - recover: {0} shavings of {1}", shavingQty,
						metalShavingsItem.Code);
#endif
				}
				else
				{
					Logger.Warning("Missing or Invalid Item: {0} ",
						MetalShavingsCode.WithPathAppendix("-" + BaseMaterial));
				}
			}
		}
	...
	internal bool WorkMatchesRecipe()
	{
		...
		var ymax = Math.Min(6, bea.SelectedRecipe.QuantityLayers); //Why ignore higher layers?
		...
		for (var z = 0; z < 16; z++)
		{
			var desiredMat = (byte)(theRecipie[x, y, z] ? EnumVoxelMaterial.Metal : EnumVoxelMaterial.Empty);
	...
	{
		if (SplitCount > 0 && bea.SelectedRecipe != null)
		{
			//Work-Item stack OK; for post split check...
			foreach (EnumVoxelMaterial voxel in bea.Voxels)
				if (voxel == EnumVoxelMaterial.Metal || voxel == EnumVoxelMaterial.Slag)
					return false;
			return true;
		}
	...
	internal bool PrefixMatcher(List<AssetLocation> nameList, AssetLocation target)
	{
		...
		foreach (var aName in nameList)
			if (target.BeginsWith(aName.Domain, aName.Path))
				return true;
...
#if DEBUG
				Logger.VerboseDebug("(old) Selected Recipe: {0} base-material: '{1}' worth {2} units; spawning",
					bea.SelectedRecipe.Output.Code, result.PrimaryMaterial, result.TotalQuantity);
#endif

				var shavingQty = (int)(result.TotalQuantity / MetalRecoverySystem.IngotVoxelDefault);

				if (shavingQty > 0)
				{
					var metalShavingsItem =
						World.GetItem(MetalShavingsCode.AppendPathVariant(result.PrimaryMaterial.PathEnding()));
